/** Reads a cancellation command only for the currently authenticated organization billing manager. */
...
import { readOrganizationSubscriptionCancellation } from "@/lib/services/subscription-cancellation";
...
app.get("/", rateLimit(RateLimitPresets.STANDARD), async (c) => {
  ...
  try {
    ...
    const data = await readOrganizationSubscriptionCancellation({
